...
const AdminPanel = () => {
  ...
  const fetchRoomStats = useCallback(async () => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/admin/room-stats', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const fetchMessageStats = useCallback(async () => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/admin/message-stats', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  }, [activeTab, fetchAllUsers, fetchRoomStats, fetchMessageStats, fetchUserActivity]);
